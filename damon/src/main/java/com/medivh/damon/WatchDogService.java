...
public class WatchDogService extends Service {
    ...
    protected final int onStart(Intent intent, int flags, int startId) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            Intent i = new Intent(Daemon.sApp, Daemon.sServiceClass);
            sPendingIntent = PendingIntent.getService(Daemon.sApp, HASH_CODE, i, PendingIntent.FLAG_UPDATE_CURRENT);
            am.setRepeating(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + Daemon.getWakeUpInterval(), Daemon.getWakeUpInterval(), sPendingIntent);
        }

        //使用定时 Observable，避免 Android 定制系统 JobScheduler / AlarmManager 唤醒间隔不稳定的情况
        sDisposable = Observable
                .interval(Daemon.getWakeUpInterval(), TimeUnit.MILLISECONDS)
        ...
        getPackageManager().setComponentEnabledSetting(new ComponentName(getPackageName(), Daemon.sServiceClass.getName()),
