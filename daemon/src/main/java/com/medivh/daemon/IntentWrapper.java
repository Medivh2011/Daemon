...
public class IntentWrapper {
    ...
    public static List<IntentWrapper> getIntentWrapperList() {
        if (sIntentWrapperList == null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                PowerManager pm = (PowerManager) Daemon.sApp.getSystemService(Context.POWER_SERVICE);
                boolean ignoringBatteryOptimizations = pm.isIgnoringBatteryOptimizations(Daemon.sApp.getPackageName());
                if (!ignoringBatteryOptimizations) {
                    Intent dozeIntent = new Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS);
                    dozeIntent.setData(Uri.parse("package:" + Daemon.sApp.getPackageName()));
            ...
            xiaomiGodIntent.putExtra("package_name", Daemon.sApp.getPackageName());
            ...
            meizuIntent.putExtra("packageName", Daemon.sApp.getPackageName());
    ...
    public static String getApplicationName() {
        if (sApplicationName == null) {
            ...
            ApplicationInfo ai;
            try {
                pm = Daemon.sApp.getPackageManager();
                ai = pm.getApplicationInfo(Daemon.sApp.getPackageName(), 0);
                sApplicationName = pm.getApplicationLabel(ai).toString();
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
                sApplicationName = Daemon.sApp.getPackageName();
            }
        }
